...
#ifdef ACCRETION
	All.Acradius = all.Acradius;
#endif // ACCRETION
	
#ifdef ANALYTIC_STAR
	All.Analytic_Mass = all.Analytic_Mass;
	All.Analytic_Soft = all.Analytic_Soft;	  
#endif // ANALYTIC_GRAVITY
       
#ifdef PLANETAC
	All.PlanetMass = all.PlanetMass;
	All.PlanetR = all.PlanetR;
	All.PlanetACtime = all.PlanetACtime;
	All.HeadWindV = all.HeadWindV;
#endif // PLANETAC

#ifdef BETACOOL
        All.betacool = all.betacool;
#endif
...
#ifdef ACCRETION
      strcpy(tag[nt], "Acradius");
      addr[nt] = &All.Acradius;
      id[nt++] = REAL;
#endif
      
#ifdef ANALYTIC_STAR
      strcpy(tag[nt], "Analytic_Mass");
      addr[nt] = &All.Analytic_Mass;
      id[nt++] = REAL;
      strcpy(tag[nt], "Analytic_Soft");
      addr[nt] = &All.Analytic_Soft;
      id[nt++] = REAL;  
#endif // ANALYTIC_GRAVITY 

      
#ifdef PLANETAC
      strcpy(tag[nt], "PlanetMass");
      addr[nt] = &All.PlanetMass;
      id[nt++] = REAL;

      strcpy(tag[nt], "PlanetR");
      addr[nt] = &All.PlanetR;
      id[nt++] = REAL;

      strcpy(tag[nt], "PlanetACtime");
      addr[nt] = &All.PlanetACtime;
      id[nt++] = REAL;

      strcpy(tag[nt], "HeadWindV");
      addr[nt] = &All.HeadWindV;
      id[nt++] = REAL;
#endif      
      
#ifdef BETACOOL
      strcpy(tag[nt], "betacool");
...
#ifdef HYDRO_SPH
    All.ArtBulkViscConst = 0.1; /*test for different alpha */
#ifdef SPHAV_ARTIFICIAL_CONDUCTIVITY
...
#ifdef HYDRO_SPH
    if((All.ArtBulkViscConst<=0.05)||(All.ArtBulkViscConst>=2.0))
    {
        if(ThisTask==0)
            printf("ArtBulkViscConst must be >0.05 and <2 to ensure stability \n");
        endrun(1);
    }
#ifdef SPHAV_ARTIFICIAL_CONDUCTIVITY
...
{
  int j;
  Mattable = malloc(4*sizeof(TILLMATERIAL *));
  for (j=0;j< 4; j++)
  {
    ...
    {
      case 0:
        Mattable[j] = tillInitMaterial(GRANITE,All.UnitLength_in_cm,All.UnitMass_in_g,1000,1000,100.0,1200.0,1);
        break;
      case 1:
        Mattable[j] = tillInitMaterial(IRON,All.UnitLength_in_cm,All.UnitMass_in_g,1000,1000,100.0,1200.0,1);
        break;
      case 2:
        Mattable[j] = tillInitMaterial(BASALT,All.UnitLength_in_cm,All.UnitMass_in_g,1000,1000,100.0,1200.0,1);
        break;
      case 3:
        Mattable[j] = tillInitMaterial(ICE,All.UnitLength_in_cm,All.UnitMass_in_g,1000,1000,100.0,1200.0,1);
        break;
      default:
        /* Unknown material */
        assert(0);
...
void init_mat_table(void)
{
#ifndef EOS_TABULATED
    char inFile[256] = "aneos.input";
    initaneos(inFile);
#else  
  char inFile[256] = "aneos.input";
  int j;
  char table1[256],table0[256],table2[256];
  Mattable = malloc(3*sizeof(ANEOSTable *));
  ...
  for (j=0;j< 2; j++)
  {
    ...
    switch(j)
    {
      case 0:
        ...
        Mattable[j] = ANEOSInitTable(0, 1e-6, 1e6, 0.01, 1e6, 1e14,0.005);
        break;
      case 1:
        //Mattable[j] = ANEOSInitTable(1, 9., 12., 0.01, 4.0e10, 8.0e10, 1.0e8);
        Mattable[j] = ANEOSInitTable(1, 1e-6, 1e6, 0.01, 1e6, 1e14,0.005);
        break;
      case 2:
        Mattable[j] = ANEOSInitTable(1, 7., 10., 0.01, 1.0e10, 6.0e10, 1.0e8);
        //        Mattable[j] = ANEOSInitTable(1, 1e-6, 1e6, 0.01, 1e6, 1e14,0.005);
        break;
      default:
        /* Unknown material */
  ...
  snprintf(table0, sizeof table0,"%s%s",All.EosTable,"0");
  snprintf(table1, sizeof table1,"%s%s",All.EosTable,"1");
  //  snprintf(table2, sizeof table1,"%s%s",All.EosTable,"2");


  ANEOSTableRead(Mattable[0],table0,1201,1601);
  ANEOSTableRead(Mattable[1],table1,1201,1601);
  //  ANEOSTableReadHires(Mattable[2],table2,301,501);
  

  if (ANEOSTableRead(Mattable[1],table1,1201,1601))
  {
    printf("Failed reading %s.\n", table1);
    assert(0);
  }

  ANEOSTableConvertCodeunits(Mattable[0], All.UnitLength_in_cm, All.UnitMass_in_g);  
  ANEOSTableConvertCodeunits(Mattable[1], All.UnitLength_in_cm, All.UnitMass_in_g);  
  //  ANEOSTableConvertCodeunits(Mattable[2], All.UnitLength_in_cm, All.UnitMass_in_g);  
  /*
  printf("\n");
  printf("rho[0][0]= %15.7E", Mattable[0]->Lookup[0][0].rho);
  printf("  rho[%i][0]= %15.7E", Mattable[0]->nTableRho, Mattable[0]->Lookup[600][0].rho);
  printf("\n");

    printf("u[0][0]  = %15.7E", Mattable[1]->Lookup[0][0].u);
    printf("  u[0][%i]  = %15.7E", Mattable[1]->nTableU, Mattable[1]->Lookup[0][800].u);
    printf("\n");


    printf("%15.7E", Mattable[1]->u_min*pow(10.0,(Mattable[1]->nTableU-1)*Mattable[1]->du));
    printf("%15.7E", Mattable[1]->Lookup[0][Mattable[1]->nTableU-1].u-Mattable[1]->u_max);
    printf("\n");*/
#endif
}
#endif

#ifdef EOS_NANEOS /*hardcoded table names and arr size*/
void init_mat_table(void)
{
