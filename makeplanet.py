...
print ('rho-ratio-at-cmb',cmbratio)
...
hresn=int(0.85*62.46/2.65089e-5)
lresn=920000#int(0.85*62.46/5.553959e-5)
hres = woma.ParticlePlanet(planet, hresn, verbosity=0)
lres = woma.ParticlePlanet(planet, lresn, verbosity=0)

mc=lres.A1_m[np.where(lres.A1_mat_id==401)]
mm=hres.A1_m[np.where(hres.A1_mat_id==400)]
mc[:]=62.46*0.85*0.3/len(mc)
mm[:]=2.65089e-5
m=np.concatenate((mc,mm))

print ('mplanet', np.sum(m)/62.46)
uc=lres.A1_u[np.where(lres.A1_mat_id==401)]
um=hres.A1_u[np.where(hres.A1_mat_id==400)]
u=np.concatenate((uc,um))
u/=1e6 #woma to code unit

tc=lres.A1_T[np.where(lres.A1_mat_id==401)]
tm=hres.A1_T[np.where(hres.A1_mat_id==400)]
temp=np.concatenate((tc,tm))
...
posc=lres.A2_pos[np.where(lres.A1_mat_id==401)]
posm=hres.A2_pos[np.where(hres.A1_mat_id==400)]
pos=np.concatenate((posc,posm))
pos/= 6.37869e+06 # woma to code unit
...
velc=lres.A2_vel[np.where(lres.A1_mat_id==401)]
