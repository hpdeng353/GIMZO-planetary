...
#ifdef BETACOOL
            double newu=0.;
            newu = SphP[i].InternalEnergy - SphP[i].InternalEnergy/5.*pow((P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]*P[i].Pos[2]),-0.75)*dtime;
            if(newu <0.5*SphP[i].InternalEnergy){SphP[i].InternalEnergy *= 0.5;} else {SphP[i].InternalEnergy = newu;}    
	...
	for(j = 0; j < 3; j++) 
        {
            ...
            P[i].Vel[j] += dp[j] / mass_new; /*mass_new correctly accounts for mass change if its allowed */
	}
