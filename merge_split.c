...
int does_particle_need_to_be_merged(int i)
{
    ...
    //#ifdef INFALL
    //        if((P[i].Type==0)&&(P[i].Mass<6.e-8))  return 1;
...
#endif
    if(P[i].Mass<0) return 1;
    
    //    if((P[i].Type==0)&&(P[i].Mass<1.e-7)&&(P[i].Hsml <0.014)) return 1;
    //    if((P[i].Type==0)&&(P[i].Mass<2.e-7)&&(P[i].Hsml <0.01)) return 1;
    //    if((P[i].Type==0)&&(P[i].Mass<4.e-7)&&(P[i].Hsml <0.01)) return 1;
    //    if((P[i].Type==0)&&(P[i].Mass<4.e-6)&&(P[i].Hsml <0.03)) return 1;
...
#endif
    //    if ((P[i].Mass>0.00009) && ((P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]*P[i].Pos[2])<0.1579*0.1579))return 1;
    /*    if (((P[i].Pos[0]-0.6)*(P[i].Pos[0]-0.6)+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]*P[i].Pos[2])<1.5*1.5)
    {
      if ((P[i].Mass>6e-5)&&(P[i].Hsml>0.1)) return 1;
      if ((P[i].Mass>3e-5)&&(P[i].Hsml>0.15)) return 1;          
    }*/
    //    if (P[i].Mass>9.e-8) return 1;
    //if ((P[i].Mass>3e-5)&&(P[i].Mass<5.68e-5)) return 1;          

    //    if((P[i].Mass>3e-5)&&(P[i].Mass<5.4e-5)) return 1;

    //    if((P[i].Type==0)&&(P[i].Mass<0.0001120)&&(P[i].Mass>0.00008)) return 1;
    //    if((P[i].Type==0)&&(P[i].Mass>2.e-8)) return 1;
    //    if((P[i].Type==0)&&(P[i].Mass>5.e-9)) return 1;
#ifdef INFALL
    //if((P[i].Type==0)&&(P[i].Mass>6e-8)&&(P[i].Hsml>0.09)) return 1;
    if((P[i].ID>InfallID)&&(P[i].ID<InfallID+4)&&(P[i].ID_child_number==0)&&(P[i].Mass< All.MinMassForParticleMerger*3)) return 1;
    //    if((P[i].ID>InfallID)&&(P[i].ID<InfallID+19)&&(P[i].ID_child_number==0)) return 1;
#endif
...
void clipping(void)
{
  ...
  {
    double rr,zz;
    rr=P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1];
    //    zz=P[i].Pos[2]*P[i].Pos[2]/rr;
    ///if((P[i].Type==0)&&(zz>0.01)) P[i].Mass=0;
    //        if((P[i].Type==0)&&(rr<1)) P[i].Mass=0;
    //    if (P[i].ID > 1742127) P[i].Mass=0; /* 0.8 for shearing box 1.5 for global disk would be fine*/
    //    if((npgrp!=2)||
    //        if(P[i].Type==1) P[i].Mass=0;
    //    if(abs(P[i].Vel[2])>1) P[i].Mass=0;

                if (P[i].Hsml > 0.8*All.MaxHsml) P[i].Mass=0; /* 0.8 for shearing box 1.5 for global disk would be fine*/

    //    if ((P[i].Type==0) && (P[i].Mass>3e-9)) P[i].Mass=0.;
    //    if (P[i].Mass>0.1) P[i].Mass=0.;
  }
}
#endif // CLIPPING
...
#if defined(ACCRETION) && !defined(ANALYTIC_STAR)
void star_accretion(void)
{
  int i,j,StarTASK[2],GlobalStarTask[2];
  int StarControl,StarControl_tot;
  double StarMass=0,StarAcMass_tot=0,StarAcMass=0,Dist2Star=0,xx,yy,zz,wt_star;
  double StarPos[3], StaroldP[3],StarAcP[3],StarAcPos[3],StarAcForce[3],StarAcMF[3];
  double StarAcPos_tot[3],StarAcP_tot[3],StarAcForce_tot[3],StarAcMF_tot[3];
  ...
  for ( i=0; i<NumPart; i++)
      ...
      if (P[i].ID == 50000000)
	{
	  ...
	  StarTASK[0] = 1;
	  StarMass = P[i].Mass;
          for(j=0;j<3;j++) StaroldP[j]=StarMass *P[i].Vel[j];
	  break;	 
	}
    }
  MPI_Allreduce(StarTASK, GlobalStarTask, 1, MPI_2INT, MPI_MAXLOC,MPI_COMM_WORLD);
  MPI_Bcast(&StarPos, 3, MPI_DOUBLE, GlobalStarTask[1], MPI_COMM_WORLD);
  MPI_Bcast(&StarMass, 1, MPI_DOUBLE,GlobalStarTask[1], MPI_COMM_WORLD);

  for (i = 0; i < NumPart; i++)
    {
      if((P[i].Type==0)&&(TimeBinActive[P[i].TimeBin]))
	{
	  xx = P[i].Pos[0] - StarPos[0];
	  yy = P[i].Pos[1] - StarPos[1];
          zz = P[i].Pos[2] - StarPos[2];
          Dist2Star = xx * xx + yy * yy + zz * zz;
#ifdef ACCRETION
          P[i].r2star2=Dist2Star;
#endif
	  if(Dist2Star < All.Acradius*All.Acradius)
	    {
	      StarAcMass += P[i].Mass;
	      for (j = 0; j < 3; j++) {
		StarAcPos[j] += P[i].Mass * P[i].Pos[j]; /* position */
                StarAcP[j] += P[i].Mass * P[i].Vel[j]; /*momentum need to take care sphpvelpred*/
		StarAcForce[j] += P[i].Mass * P[i].GravAccel[j];/* force*/
		/*		StarAcMF[j] += P[i].B[j];  which variable to conserve */
	      }
	      P[i].Mass = 0;
	    }
	}
    }
  MPI_Allreduce(&StarAcPos, &StarAcPos_tot, 3, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&StarAcP, &StarAcP_tot, 3, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&StarAcForce, &StarAcForce_tot, 3, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&StarAcMass, &StarAcMass_tot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  /*  MPI_Allreduce(&StarAcMF, &StarAcMF_tot, 3, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);*/
  wt_star = StarMass / (StarMass + StarAcMass_tot);
  StarMass += StarAcMass_tot;
  for (i = 0; i < NumPart; i++)
    {
      if (P[i].ID == 50000001)
	{
	  ...
	  for (j = 0; j < 3; j++)
	    {
	      P[i].Pos[j] = StarPos[j] * wt_star + StarAcPos_tot[j]/StarMass;
	      P[i].Vel[j] = P[i].Vel[j] * wt_star + StarAcP_tot[j]/StarMass;
              //	      P[i].dp[j] += StarAcP_tot[j];
              P[i].dp[j] = P[i].dp[j] + StarMass*P[i].Vel[j] - StaroldP[j];
	      P[i].GravAccel[j] = P[i].GravAccel[j] * wt_star + StarAcForce[j]/StarMass;
	    }
	  break;
  ...
  if (ThisTask == 0 ) 
    {
      printf("debug: mass accreted by star %14.6f, at  %14.6f, %14.6f, %14.6f\n", StarMass,StarPos[0],StarPos[1],StarPos[2]);
...
} // ACCRETION sink particle star Partype1

#endif


#if defined(ACCRETION) && defined(ANALYTIC_STAR)
void Analytic_accretion(void) /* add mass to (0,0,0)*/
{    
  int i;
  double Dist2Star=0, StarAcMass=0, StarAcMass_tot=0;
  for (i = 0; i < NumPart; i++)
    {
      if((P[i].Type==0)&&(TimeBinActive[P[i].TimeBin]))
	{
	  Dist2Star = P[i].Pos[0] * P[i].Pos[0] + P[i].Pos[1] * P[i].Pos[1] + P[i].Pos[2] * P[i].Pos[2];
	  if(Dist2Star < All.Acradius * All.Acradius)
	    {
              StarAcMass += P[i].Mass;
	      P[i].Mass = 0;
  ...
  MPI_Allreduce(&StarAcMass, &StarAcMass_tot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  All.Analytic_Mass += StarAcMass_tot;
               ...
               /* if merging: do a neighbor loop ON THE SAME DOMAIN to determine the target */
                ...
                if(numngb_inbox>0)
                {
                    ...
                    for(n=0; n<numngb_inbox; n++)
                    {
                        ...
                        if((j>=0)&&(j!=i)&&(P[j].Type==P[i].Type)&&(P[i].Mass==P[j].Mass)) //hpdeng added to decrease resolution in clumps
                        {
...
#else
          if((P[i].Type==0)&&(TimeBinActive[P[i].TimeBin])&&(All.HighestActiveTimeBin == All.HighestOccupiedTimeBin))
#endif
        {
