...
static struct densdata_in
{
  ...
  int Type;
#if (defined SepMat) || (defined CMboundary)
  int imat;
#endif
#ifdef GDSPH
  MyDouble temprho;
...
static struct densdata_out
{
    ...
    MyLongDouble Rho;
#ifdef SBETACOOL
    MyLongDouble delta_u;
#endif

#ifdef RhoCorr
  MyLongDouble fx1,fy1,fz1,fh,ironnumb1; 
#endif
...
#ifdef SBETACOOL
        ASSIGN_ADD(SphP[i].DeltaU, out->delta_u, mode);
#endif

#ifdef RhoCorr
        ASSIGN_ADD(SphP[i].fx, out->fx1, mode);
        ASSIGN_ADD(SphP[i].fy, out->fy1, mode);
        ASSIGN_ADD(SphP[i].fz, out->fz1, mode);
        ASSIGN_ADD(SphP[i].ironnumb, out->ironnumb1, mode);
        //        ASSIGN_ADD(SphP[i].Vfrac, out->fh, mode);
#endif  
        for(k = 0; k < 3; k++)
            for(j = 0; j < 3; j++)
...
#ifdef GDSPH
	ASSIGN_ADD(SphP[i].SymfU, out->SymfU1, mode);
	ASSIGN_ADD(SphP[i].SymfL, out->SymfL1, mode);
#endif
    ...
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        if(density_isactive(i))
        {
            if(P[i].Type == 0 && P[i].Mass > 0)
            {
...
#else
                        SphP[i].EgyWtDensity /= SphP[i].Pressure; //hpdeng added following Hosono2013;
#endif
                    } else {
                ...
                orelax_isentropic_pin(i); /* fixed-entropy relaxation: reset u onto the initial isentrope before the pressure evaluation (no-op unless RelaxIsentropic=1) */
#endif
	    ...
	    if(SphP[i].temprho >0)
            {
              SphP[i].Symf=SphP[i].SymfU/SphP[i].SymfL;
            }else
            {
              SphP[i].Symf =1;
            }
...
#ifdef RhoCorr   //for mantaining core mantle bounday discontinuity
            //            SphP[i].Vfrac = 1.; //sqrt(SphP[i].fx * SphP[i].fx + SphP[i].fy * SphP[i].fy + SphP[i].fz * SphP[i].fz)/2./PPP[i].Hsml/SphP[i].Vfrac;
	    /*	    if (SphP[i].Vfrac > 0.27)
            {
              SphP[i].Vfrac = 0.344;
            }
            else if (SphP[i].Vfrac < 0.01)
            {
              SphP[i].Vfrac = 1.0;
            }
            else
            {
              SphP[i].Vfrac = 1.0259 - 2.52444*SphP[i].Vfrac;
            }
            if (SphP[i].ironnumb == 0) SphP[i].Density /= SphP[i].Vfrac;*/
#endif


#if defined(Density_Corr) && (KERNEL_FUNCTION == 7)
	    SphP[i].Density -= 0.01342 * P[i].Mass * 495./32./3.1415926 * pow((All.DesNumNgb/100.),-1.579)/P[i].Hsml/P[i].Hsml/P[i].Hsml;
#if defined(TWODIMS)
	    SphP[i].Density -= 0.0044;
#endif
...
#else
          numngb_inbox = ngb_treefind_variable_threads_targeted(local.Pos, local.Hsml, target, &startnode, mode, exportflag, exportnodecount, exportindex, ngblist, local.imat);                  
#endif      
            ...
            for(n = 0; n < numngb_inbox; n++)
            {
                ...
                if(r2 < h2)
                {
...
#ifdef MARKNGB
                    if(local.ID==799834) {P[j].ngbto=1;}
                    if(local.ID==290281) {P[j].ngbto=2;}
                    if(local.ID==291678) {P[j].ngbto=3;}
#endif
                    ...
                    double rhoratio;
                    rhoratio=1.;
                    if(SphP[j].imat > local.imat){rhoratio=0.5;}
                    if(SphP[j].imat < local.imat){rhoratio = 2.;}
                    out.Ngb += kernel.wk * rhoratio;
                    out.Rho += kernel.mj_wk * rhoratio;
#endif

#ifdef SBETACOOL
                    out.delta_u += FLT(mass_j * kernel.wk /SphP[j].temprho*SphP[j].InternalEnergy);
#endif

#ifdef RhoCorr
                    out.fx1 += FLT(kernel.dp[0] * kernel.mj_wk);
                    out.fy1 += FLT(kernel.dp[1] * kernel.mj_wk);
                    out.fz1 += FLT(kernel.dp[2] * kernel.mj_wk);
                    out.ironnumb1 += 1; //SphP[j].imat;
                    out.fh += kernel.mj_wk;
#endif
		    ...
		    if (u>1e-5) 
		      {
			out.SymfU1 += FLT(kernel.dwk*mass_j)/u*r2/local.temprho;
			out.SymfL1 += FLT(kernel.dwk*mass_j)/u*r2/SphP[j].temprho;
		      }
