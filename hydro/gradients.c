...
#if defined (SPHAV_CD10_VISCOSITY_SWITCH) || defined(SPHAV_HU14_VISCOSITY_SWITCH)
    MyFloat NV_DivVel;
#endif
...
void hydro_gradient_calc(void)
{
...
#ifdef SPHAV_HU14_VISCOSITY_SWITCH
    double NV_dt,NV_A,h_eff,alphaloc,divVel_physical;
#endif
    ...
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
        if(P[i].Type==0)
        {
            ...
            SphP[i].divB = 0;
#endif
#ifdef SPHAV_CD10_VISCOSITY_SWITCH               /* not sure*/
...
#pragma omp parallel
    ...
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
	    ...
	    divVel_physical = All.cf_a2inv * fabs(SphP[i].Gradients.Velocity[0][0] + SphP[i].Gradients.Velocity[1][1] + SphP[i].Gradients.Velocity[2][2]);/* SphP[i].NV_DivVel;*/
            
            // add a simple limiter here: alpha_loc is 'prepped' but only switches on when the divergence goes negative: want to add hubble flow here //
            if(All.ComovingIntegrationOn) divVel_physical += 3*All.cf_hubble_a; // hubble-flow correction added
	    NV_A = DMAX(-SphP[i].NV_dt_DivVel, 0.0);
	    /*            if(divVel_physical>=0.0) NV_A = 0.0;*/
	    NV_A = divVel_physical;
	    ...
	    h_eff = Get_Particle_Size(i) * All.cf_atime; // 'default' parameter choices are scaled for a cubic spline //
	    SphP[i].alpha_limiter = divVel_physical*divVel_physical/(divVel_physical*divVel_physical + MagCurl*MagCurl + 0.0001*Particle_effective_soundspeed_i(i)*Particle_effective_soundspeed_i(i) /
					(Get_Particle_Size(i)) /(Get_Particle_Size(i)) / fac_mu / fac_mu);
	    alphaloc = h_eff*h_eff*NV_A / (Particle_effective_soundspeed_i(i)*Particle_effective_soundspeed_i(i) + h_eff*h_eff*NV_A);

	    NV_dt =  (P[i].TimeBin ? (((integertime) 1) << P[i].TimeBin) : 0) * All.Timebase_interval / All.cf_hubble_a; // physical
	    if(SphP[i].alpha < alphaloc)
                SphP[i].alpha = alphaloc;
            else if (SphP[i].alpha > alphaloc)
	      SphP[i].alpha = alphaloc + (SphP[i].alpha - alphaloc) * exp(-NV_dt * (fabs(SphP[i].MaxSignalVel)*All.cf_afac3)/h_eff/10.);

	    /*    if(SphP[i].alpha*SphP[i].alpha_limiter < 0.05)
	      {
		SphP[i].alpha = 0.05;
		SphP[i].alpha_limiter = 1.;
		}*/
...
#if defined(CONDUCTION_SPITZER) || defined(VISCOSITY_BRAGINSKII) //|| defined(MHD_NON_IDEAL_TODO)
