...
#ifdef GDSPH
    hfc_i = 0.5*local.Symf;
    hfc_j = 0.5*SphP[j].Symf;
#endif
...
#ifdef GDSPH
    kernel.dwk_ij = hfc_i * kernel.dwk_i +hfc_j * kernel.dwk_j;
    hfc *= (local.Pressure+SphP[j].Pressure)/local.Density/SphP[j].Density;
...
#else
    Fluxes.p += local.Pressure/local.Density/SphP[j].Density * kernel.dwk_ij * local.Mass * P[j].Mass / kernel.r  * vdotr2_phys - hfc * vi_dot_r; /* total energy  the second term is the kinetic energy variation*/
#endif    
...
#else
#ifndef SPHAV_HU14_VISCOSITY_SWITCH	
        double BulkVisc_ij = 0.5 * All.ArtBulkViscConst * (local.alpha + SphP[j].alpha_limiter);
#else
	double BulkVisc_ij = 0.5 * All.ArtBulkViscConst * (local.alpha + SphP[j].alpha_limiter * SphP[j].alpha);
#endif
