...
#if (!defined(FIRE_UNPROTECT_FROZEN) || (SLOPE_LIMITER_TOLERANCE == 0)) && (SLOPE_LIMITER_TOLERANCE != 2) || defined(BETACOOL) 
#if defined(COOLING) || (SLOPE_LIMITER_TOLERANCE==0) || defined(BETACOOL) || defined(EOS_GENERAL)
...
*/
...
#endif
#ifdef LorentzOff
        double press_tot_limiter1;
        press_i_tot = local.Pressure + local.Density * v2_approach;
        press_j_tot = SphP[j].Pressure + SphP[j].Density * v2_approach;
        press_tot_limiter1 = 1.1 * All.cf_a3inv * DMAX( press_i_tot , press_j_tot );
#endif        
#ifdef EOS_GENERAL
	...
	//        press_tot_limiter = 1.e10*(press_tot_limiter+1.); // it is unclear how this particular limiter behaves for solid-body EOS's, so for now, disable it in these cases
        ...
        /* --------------------------------------------------------------------------------- */
#ifdef MOONtest        //hpdeng the method in x y hu 2009 not used !!
        Riemann_vec.L.imat=local.imat; Riemann_vec.R.imat=SphP[j].imat;
        Riemann_vec.R.eospsi=local.eospsi; Riemann_vec.L.eospsi = SphP[j].eospsi;
        Riemann_vec.R.eosgamma=local.eosgamma; Riemann_vec.L.eosgamma = SphP[j].eosgamma;
#endif

#ifdef LorentzOff
        struct Input_vec_Riemann Riemann_vec1;
        Riemann_vec1.L.rho = Riemann_vec.L.rho;
        Riemann_vec1.L.p = Riemann_vec.L.p;
        Riemann_vec1.L.v[0] =Riemann_vec.L.v[0];
        Riemann_vec1.L.v[1] =Riemann_vec.L.v[1];
        Riemann_vec1.L.v[2] =Riemann_vec.L.v[2];
        Riemann_vec1.L.u =Riemann_vec.L.u;
        Riemann_vec1.L.cs =Riemann_vec.L.cs;

        Riemann_vec1.R.rho = Riemann_vec.R.rho;
        Riemann_vec1.R.p = Riemann_vec.R.p;
        Riemann_vec1.R.v[0] =Riemann_vec.R.v[0];
        Riemann_vec1.R.v[1] =Riemann_vec.R.v[1];
        Riemann_vec1.R.v[2] =Riemann_vec.R.v[2];
        Riemann_vec1.R.u =Riemann_vec.R.u;
        Riemann_vec1.R.cs =Riemann_vec.R.cs;
#endif
...
#ifdef LorentzOff
        Riemann_solver1(Riemann_vec1, &Riemann_out, n_unit, press_tot_limiter1);
        /* before going on, check to make sure we have a valid Riemann solution */

        if((Riemann_out.P_M<0)||(isnan(Riemann_out.P_M))||(Riemann_out.P_M>1.4*press_tot_limiter1))
        {
            /* go to a linear reconstruction of P, rho, and v, and re-try */
            Riemann_vec1.R.p = local.Pressure; Riemann_vec1.L.p = SphP[j].Pressure;
            Riemann_vec1.R.rho = local.Density; Riemann_vec1.L.rho = SphP[j].Density;
            for(k=0;k<3;k++) {Riemann_vec1.R.v[k]=local.Vel[k]-v_frame[k]; Riemann_vec1.L.v[k]=VelPred_j[k]-v_frame[k];}
      
            Riemann_solver1(Riemann_vec1, &Riemann_out, n_unit, 1.4*press_tot_limiter1);
            ...
            if((Riemann_out.P_M<0)||(isnan(Riemann_out.P_M)))
            {
                ...
                Riemann_vec1.R.p = local.Pressure; Riemann_vec1.L.p = SphP[j].Pressure;
                Riemann_vec1.R.rho = local.Density; Riemann_vec1.L.rho = SphP[j].Density;
                for(k=0;k<3;k++) {Riemann_vec1.R.v[k]=0; Riemann_vec1.L.v[k]=0;}

                Riemann_solver1(Riemann_vec1, &Riemann_out, n_unit, 2.0*press_tot_limiter1);

                if((Riemann_out.P_M<0)||(isnan(Riemann_out.P_M)))
                {
                    printf("Riemann Solver1 Failed to Find Positive Pressure!: Pmax=%g PL/M/R=%g/%g/%g Mi/j=%g/%g rhoL/R=%g/%g vL=%g/%g/%g vR=%g/%g/%g n_unit=%g/%g/%g \n",
                           press_tot_limiter1,Riemann_vec1.L.p,Riemann_out.P_M,Riemann_vec1.R.p,local.Mass,P[j].Mass,Riemann_vec1.L.rho,Riemann_vec1.R.rho,
                           Riemann_vec1.L.v[0],Riemann_vec1.L.v[1],Riemann_vec1.L.v[2],
                           Riemann_vec1.R.v[0],Riemann_vec1.R.v[1],Riemann_vec1.R.v[2],n_unit[0],n_unit[1],n_unit[2]);
                    exit(1234); 
                }
            }
        } // closes loop of alternative reconstructions if invalid pressures are found //
...
#if (SLOPE_LIMITER_TOLERANCE < 2) && !defined(MOONOLD)
...
#if defined(MAGNETIC) && !defined(LorentzOff)
