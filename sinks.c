...
void do_sinks(void)
{
  ...
  MPI_Allreduce(&NumSinks, &All.TotNumSinks, 1, MPI_INT, MPI_SUM, MYMPI_COMM_WORLD);
  ...
  if(All.TotNumSinks > 0)
	...
	terminate("Failed to allocate memory!\n");
      ...
      MPI_Allgather(&NumSinks, 1, MPI_INT, count_list, 1, MPI_INT, MYMPI_COMM_WORLD);
      ...
      for(i = 0, index = prev_list[ThisTask]; i < NumSinks; i++)
	{
...
#ifdef MAGNETIC
	      sink_send[index].bfld[j] = SphP[NumPart - NumSinks + i].b2.BPred[j];
#endif
	    }
      ...
      MPI_Allgatherv(&sink_send[prev_list[ThisTask]], NumSinks * sizeof(struct sink_list), MPI_BYTE, sink,
		     count_list_bytes, prev_list_bytes, MPI_BYTE, MYMPI_COMM_WORLD);
		  ...
		  if(r > h)
		  ...
		  else
		    {
		      u = r / h;
                      ...
                      double uu = u*u;
                      wp = -1.0 / h * 0.0625 * (-55 + uu*(165 + uu*(-462 + uu*(1650 + u*(-2640 + u*(1925 + u*(-704 + 105*u)))))));
		    }
  ...
  if(All.TotNumSinks > 0)
	...
	memmove(P + N_gas, P + N_gas + num_acc, (NumPart - N_gas) * sizeof(struct particle_data));
      ...
      MPI_Allreduce(&num_acc, &num_acc_glob, 1, MPI_INT, MPI_SUM, MYMPI_COMM_WORLD);

      if(num_acc_glob)
	{
	  MPI_Allreduce(mass_array, glob_mass_array, All.TotNumSinks, MPI_DOUBLE, MPI_SUM, MYMPI_COMM_WORLD);
	  MPI_Allreduce(cm_array, glob_cm_array, 3 * All.TotNumSinks, MPI_DOUBLE, MPI_SUM, MYMPI_COMM_WORLD);
	  MPI_Allreduce(mom_array, glob_mom_array, 3 * All.TotNumSinks, MPI_DOUBLE, MPI_SUM, MYMPI_COMM_WORLD);
  ...
  MPI_Allreduce(&local, &global, 1, MPI_DOUBLE_INT, MPI_MAXLOC, MYMPI_COMM_WORLD);
  ...
  if(global.nh_max > All.SinkDensThresh)
	...
	for(i = 0; i < 3; i++)
      ...
      MPI_Bcast(sink_pos, 3, MPI_DOUBLE, global.task, MYMPI_COMM_WORLD);
      ...
      for(i = count = 0; i < N_gas; i++)
	{
      ...
      MPI_Allgather(&count, 1, MPI_INT, count_list, 1, MPI_INT, MYMPI_COMM_WORLD);
      ...
      if(count_global)
	{
...
#ifdef MAGNETIC
		      sink_send[index].bfld[j] = SphP[i].b2.BPred[j];
#endif
		    }
	  ...
	  if(ThisTask == 0)
	    ...
	    printf("Sink Check: %d particles, egrav = %g, ekin = %g, etherm = %g, divv = %g\n", count_global,
		   e_grav, e_kin, e_therm, divv);
#else
	    printf("Sink Check: %d particles, egrav = %g, ekin = %g, etherm = %g, emag = %g, divv = %g\n",
		   count_global, e_grav, e_kin, e_therm, e_mag, divv);
#endif
	  ...
	  if(e_grav > (2 * e_therm + e_mag) && e_grav > e_kin + e_therm + e_mag && divv < 0)
		...
		memmove(P + N_gas, P + N_gas + num_acc, (NumPart - N_gas) * sizeof(struct particle_data));
	      ...
	      if(num_acc_glob)
		{
		  MPI_Allreduce(&numforceupdate, &globnumforceupdate, 1, MPI_INT, MPI_SUM, MYMPI_COMM_WORLD);
  ...
  MPI_Allreduce(&num_update, &num_update_glob, 1, MPI_INT, MPI_SUM, MYMPI_COMM_WORLD);
