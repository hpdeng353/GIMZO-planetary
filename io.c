...
#if defined(OUTPUTPOTENTIAL)  || defined(FLAG_NOT_IN_PUBLIC_CODE)
            for(n = 0; n < pc; pindex++)
              if(P[pindex].Type == type)
                {
        ...
        case IO_ACCEL:		/* acceleration */
            ...
            for(n = 0; n < pc; pindex++)
                if(P[pindex].Type == type)
                {
                  ...
                  {
                    for(k = 0; k < 3; k++)
                      fp[k] +=0; //SphP[pindex].HydroAccel[k];
                  }
                  fp += 3;
                  n++;
                }
        ...
        case IO_DTENTR:		/* rate of change of internal energy */
            ...
            for(n = 0; n < pc; pindex++)
                if(P[pindex].Type == type)
                {
                  ...
                  *fp++ = SphP[pindex].Eta_ShearViscosity;
                    n++;
                }
#endif
            break;
...
#ifdef SBETACOOL
        case IO_DELTAU:
            strncpy(label, "DELU", 4);
            break;
#endif
#ifdef RhoCorr
       case IO_VFRAC:
            strncpy(label, "VFRC", 4);
            break;
#endif
        case IO_NE:
            strncpy(label, "NE  ", 4);
