...
shellw=np.arange(0,0.2,0.0001)

binentropy=np.arange(0,1.,0.01)
binmixing=np.arange(0,1.,0.01)
binu=np.arange(0,1.,0.01)
binrho=np.arange(0,1.,0.01)
binjz=np.arange(0,1.,0.01)
binimpf=np.arange(0,1.,0.01)
binimpfc=np.arange(0,1.,0.01)
bincore=np.arange(0,1.,0.01)
r1=0
for i in range(0,len(bins)-1):
    for j in range(0,len(shellw)):
        r2=r1+shellw[j]
        massenclose=np.sum(mass[np.where((pgrp==2)&(Radius<r2))])/62.46
        if(massenclose> bins[i+1]):
            break
   
    binentropy[i]=np.average(entropy[np.where((pgrp==2)&(Radius>r1)&(Radius<r2))])
    binu[i]=np.average(u[np.where((pgrp==2)&(Radius>r1)&(Radius<r2))])
    binjz[i]=np.average(jz[np.where((pgrp==2)&(Radius1>r1)&(Radius1<r2))])
    binrho[i]=np.average(rho[np.where((pgrp==2)&(Radius>r1)&(Radius<r2))])
    binmixing[i]=np.sum(mass[np.where((pgrp==2)&(Radius>r1)&(Radius<r2)&(ids<id0))])/np.sum(mass[np.where((pgrp==2)&(Radius>r1)&(Radius<r2))])
    binimpf[i]=np.sum(mass[np.where((pgrp==2)&(Radius<r2)&(Radius>r1)&(ids>id0)&(mat==0))])/62.46/0.2/0.7
    binimpfc[i]=np.sum(mass[np.where((pgrp==2)&(Radius<r2)&(ids>id0)&(mat==0))])/62.46/0.2/0.7
    bincore[i]=np.sum(mass[np.where((pgrp==2)&(Radius<r2)&(mat==1))])/62.46/0.3
    r1=r2
