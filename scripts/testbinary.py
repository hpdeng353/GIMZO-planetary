...
def randparticle(numpart,rho_power,temp_power,rin,rout,h0,c_s):
    ...
    for i in range(0,10000):
        sigma[i] =  m.pow((rr[i]/rin),rho_power) * 2.0 *np.pi * rr[i] * m.pow((rr[i]/rin),(temp_power+3.0)/2.0) * h0 * m.sqrt(2.0 * np.pi) #note the final term corespond to the geometry factor 2*pi*r*rho
        rho[i] =  m.pow((rr[i]/rin),rho_power) * rr[i]
        normal += step * sigma[i]
...
#        phi1=phi(x1,y1)
        ...
        rho_x1_y1 =  x1* m.pow((x1/rin),rho_power)* m.exp(-y1*y1/2./h1/h1)
...
#    mu=1.0
    ...
    Rin = 0.5
    Rout = 5. #in code unit corespond to 62.5AU
    diskmass = 0.01
#    soft =0.02
...
#    T0=280
    ...
    print 'aspect radio at Rout', aspect1,"at rin", Aspect0
    print 'selfgravity=',selfgravity,'diskmass/msun=',diskmass/StarMass
