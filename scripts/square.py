...
def makeIC_box():
    ...
    rho_desired = 1. # box average initial gas density
    P_desired = 1 # initial gas pressure
    vgrainrms = 1.0 # rms velocity of collisionless particles
    gamma_eos1 = 1.4 # polytropic index of ideal equation of state the run will assume
    gamma_eos2=2.


    ample=0. #ampliture of velocity perturbation
    ...
    x0=np.arange(0.,1.,1./N_1D); x0+=0.5*(1.-x0[-1]); # shift it so the lattice is distributed in the center of box instead of occupying oneside.
    ...
    yv_g=yv_g.flatten()*Lbox # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!r z shearing box
    zv_g=zv_g.flatten()*Lbox
    # set the initial velocity in x/y/z directions (here zero)
    vx_g=0.*xv_g; vy_g=0*xv_g; vz_g=0.*xv_g #-1.5*(xv_g-0.5); # r z phi
    ...
    mv_g/=2
    uv_g=P_desired/((gamma_eos1-1.)*rho_desired) + 0.*xv_g
    # set the gas IDs: here a simple integer list
    uv_g[:]=2

    uv_g[np.where((xv_g>0.25)&(xv_g<0.75)&(yv_g>0.25)&(yv_g<0.75))]=1.
#    uv_g[np.where((xv_g>0.875)|(xv_g<0.125)|(yv_g>0.875)|(yv_g<0.125))]*=0.9


    mv_g[np.where((xv_g>0.25)&(xv_g<0.75)&(yv_g>0.25)&(yv_g<0.75))]*=2
    id_g=np.arange(1,Ngas+1)
    imat=np.arange(1,Ngas+1)
    imat[:]=0
    temp=np.arange(1,Ngas+1)
    temp[:]=4000.
    imat[np.where((xv_g>0.25)&(xv_g<0.75)&(yv_g>0.25)&(yv_g<0.75))]=1

#    vz_g += ample*v0*np.sin(Kz*yv_g)*sin #ample*np.random.uniform(-1.,1.,Ngas) #
#  vx_g = ample*v0*np.sin(Kz*yv_g)*cos #ample*np.random.uniform(-1.,1.,Ngas) #
#    vy_g = ample*v0*sin(Kz*yv_g)                                  #initial vy so that second order mag pressure will not destroy density distribution
#    yv_g = ample
    vx_g= ample*np.random.uniform(-1.,1.,Ngas) +1.
    vy_g= ample*np.random.uniform(-1.,1.,Ngas) +1.
#    vx_g[np.where((xv_g<0.75)&(xv_g>0.25)&(yv_g<0.75)&(yv_g>0.25))]=0
 #   vy_g[np.where((xv_g<0.75)&(xv_g>0.25)&(yv_g<0.75)&(yv_g>0.25))]=0
#    vx_g[np.where((xv_g>0.75)|(xv_g<0.25)|(yv_g>0.75)|(yv_g<0.25))]=0
 #   vy_g[np.where((xv_g>0.75)|(xv_g<0.25)|(yv_g>0.75)|(yv_g<0.25))]=0
#    vy_g[:]=0.05*np.sin(yv_g*2.*np.pi*0.109)
