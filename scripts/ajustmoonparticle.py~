...
s = h5py.File('/scratch/snx3000/hpdeng/gizmout/infall1/init/infall1-ics.hdf5','r+')
...
anulus1=gmass[np.where((Radius>8)&(Radius<10))]
anulus2=anulus1

x=pos[:,0]
y=pos[:,1]
z=pos[:,2]
x1=x[np.where((Radius>8)&(Radius<10))]
x2=x1

x1=0-x1
x2=0-x2

y1=y[np.where((Radius>8)&(Radius<10))]
y2=y1
z1=z[np.where((Radius>8)&(Radius<10))]
z1+=1
z2=z1-2
...
vy=vel[:,1]
vz=vel[:,2]
vx1=vx[np.where((Radius>8)&(Radius<10))]
#vx1*=2
vx2=vx1
vy1=vy[np.where((Radius>8)&(Radius<10))]
#vy1*=2
vy1=0-vy1
vy2=vy1
vz1=vz[np.where((Radius>8)&(Radius<10))]
vz1 -= 0.333
vz2=vz1+0.66

u1=u[np.where((Radius>8)&(Radius<10))]
u2=u1
