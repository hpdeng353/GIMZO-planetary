...
def randparticle(numpart,rin,rout,rs):
    import random
...
def makedisk():
    ...
    q=np.zeros((Nstar,3))
    tx=Radius/Rout*concentration
    fcx=np.log(1+tx)-tx/(1+tx)
    xfc=Radius/Rout*(np.log(1+concentration)-concentration/(1+concentration))

    v1=vvir*np.sqrt(fcx/xfc)  #halo contribution
    mdisk=StarMass*(1-np.exp(-Radius/Rs1)-Radius/Rs1*np.exp(-Radius/Rs1)) #inclosed disk mass
    mdisk*=0
    vmag=np.sqrt(v1*v1+mdisk/Radius)
    q[:,0] = -vmag*np.sin(Phi)
    q[:,1] = vmag*np.cos(Phi)
    ...
    Radius,Theta,Phi=randparticle(Ngas,Rin,Rout,Rs)
