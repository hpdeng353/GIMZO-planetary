...
def makeIC_box():
    ...
    N_1D=64; # 1D particle number (so total particle number is N_1D^DIMS)
    ...
    vx_g=0.*xv_g;vz_g=0.*xv_g;vy_g=0.*xv_g; #vy_g=-1.5*(xv_g-0.5*Lx); 
    ...
    B0=np.sqrt(8*np.pi/100.)#*np.sqrt(2.)
    beta=B0*B0/8./np.pi
    beta=1./beta
    Qz=np.pi*2*VaKz/Kz*rz/Lz
    print "beta=",beta,"Qz=",Qz
    
    bx_g=B0*np.sin(zv_g*2*np.pi*nx)
    by_g=B0*np.cos(zv_g*2*np.pi*nx)

#    deltavy=np.sin(xv_g*2*np.pi*nx)*ample
#    vy_g += deltavy
    
 #   deltap=ample/np.pi/nx*np.cos(2*np.pi*nx*xv_g)
#    uv_g=uv_g - deltap/((gamma_eos-1.)*rho_desired)
...
#    bx_g=ample*np.random.uniform(-1.,1.,Ngas)*B0
 #   by_g=ample*np.random.uniform(-1.,1.,Ngas)*B0
#    vx_g=ample*np.random.uniform(-1.,1.,Ngas)
#    vy_g+=ample*np.random.uniform(-1.,1.,Ngas)
