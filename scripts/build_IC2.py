...
def phi(rr,zz):
    potential=-1./(m.sqrt(rr*rr+zz*zz)-2.)
    return potential
    
def randparticle(numpart,rho_power,temp_power,rin,rout,rho0,beta,c_s,h0):   #beta goes to infinite reduce to zero BF +
    import random
    ...
    for i in range(0,10000):
        ...
        for j in range(0,600):
            phi1=phi(rr[i],zz[j])
            normal1 += rho0*m.exp((phi0-phi1)/c_s/c_s*m.pow((rr[i]/rin),-temp_power)*beta/(1.+beta)) * taper2(rr[i],rin,rout) * m.pow((rr[i]/rin),rho_power)*step*stepz * 2. * np.pi * rr[i]

    for i in range(0,10000):
        rho[i] = rho0*taper2(rr[i],rin,rout) * m.pow((rr[i]/rin),rho_power) * rr[i]
    ...
    while count < numpart :
        ...
        phi0=phi(x1,0)
        phi1=phi(x1,y1)
        rho_x1_y1 = rho0*taper2(x1,rin,rout) * x1* m.pow((x1/rin),rho_power)* m.exp((phi0-phi1)/c_s/c_s*m.pow((rr[i]/rin),-temp_power)*beta/(1.+beta))
        if rho_x1_y1 > z1 :
            radius.append(x1)
            z.append(y1)
            count += 1
            if (count%1000000 ==0):
                print count/1000000,'M particles'
    radius=np.asarray(radius)
    z=np.asarray(z)
    print 'maxr=',max(radius),'minr=',min(radius)
        
    theta=np.random.uniform(0,2 * np.pi,numpart)
    return radius,theta,z,normal1
...
def makedisk():
    ...
    Rin = 1 #in code unit corespond to 1.25AU temp0=300
    Rout = 25 #in code unit corespond to 62.5AU
    V_k = m.sqrt(Rin)/(Rin-2.)
    C_s = m.sqrt(T0)
    beta =15.
    H0 = 0.7
    rho0=10.
    ...
    Radius,Theta,Z, Normal1 = randparticle(Ngas,Rho_Power,Temp_Power,Rin,Rout,rho0,beta,C_s,H0)

    print 'beta=', beta,'gamma=',gamma,"totalmass=",Normal1
    print 'sigmapower=', Sigma_Power
    ...
    Vt = np.zeros(Ngas)
    for i in range(0,len(Radius)):
        Vt[i] =  Radius[i]/(Radius[i]-2)/(Radius[i]-2) + 2.0 / beta * C_s * C_s * m.pow((Radius[i]/Rin),Temp_Power) + \
        ...
        if (Radius[i] < Rout*0.95) and (Radius[i] > Rin*1.001) :
            Vt[i] = Vt[i] + (1.0 + beta) / beta * C_s * C_s * m.pow((Radius[i]/Rin),Temp_Power) * \
                    dr_taper2(Radius[i],Rin,Rout)/taper2(Radius[i],Rin,Rout)*Radius[i]
    ...
    for i in range(0,len(Radius)): # bf in gauss
         Bt[i]= rho0 * m.exp((phi(Radius[i],0)-phi(Radius[i],Z[i]))/C_s/C_s*m.pow((Radius[i]/Rin),-Temp_Power)*beta/(1.+beta))* taper2(Radius[i],Rin,Rout)*m.pow((Radius[i]/Rin),Rho_Power)
         Bt[i]= m.sqrt(8.0 * np.pi *1.5e14 /beta *Bt[i])*C_s * m.pow(Radius[i]/Rin,Temp_Power/2.0)
