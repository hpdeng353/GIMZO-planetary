...
s = h5py.File('/scratch/snx3000/hpdeng/gizmout/grvdisk5/init/grvdisk5-ics.hdf5','r+')

#temp = s["PartType0"]["Temperature"][:]
#rho = s["PartType0"]["Density"][:]
#del s["PartType0"]["Temperature"]
#mass1= s["PartType1"]["Masses"][:]
...
#mass *=0.8
#u*=4.
#np.savetxt("rhomfm.txt",rho)
#np.savetxt("Smfm.txt",entr)
#np.savetxt("tempmfm.txt",temp)
#mass[:]=0.00002814
...
#mass[np.where(mat==0)]=0.00005584
#mass[np.where(mat==0)]=0
...
#u[np.where(mat==1)]=6
#u[np.where(mat==0)] =2
#mag[:,2]=0.5*np.sin(2*np.pi*xx/1.414)
#vel[:,1]=0.1126*np.cos(2*np.pi*xx/1.414)
#u[np.where(radius<0.02)]=300
#u[np.where(u<1)]=1.
...
#s.create_dataset("PartType0/VectorPotential",data=vel)
