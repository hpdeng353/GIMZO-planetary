...
StarMass = 10.0
Rho_Power = -1.5 # diff from sigma power rho_per
Temp_Power = -1.0
...
for i in range(0,1000):
    if (rr[i] < Rout*0.95) and (rr[i]>Rin*1.001) : 
        Vt = V_k * V_k * Rin / rr[i] + 2.0 / beta * C_s * C_s * m.pow((rr[i]/Rin),Temp_Power) + \
             ...
             dr_taper2(rr[i],Rin,Rout)/taper2(rr[i],Rin,Rout)*rr[i]
    ...
    for j in range(0,800):
        normal += m.exp(1./Aspect0/Aspect0 * m.pow((rr[i]/Rin),(-Temp_Power-1.))*(1.0/m.sqrt(1+zz[j]*zz[j]/rr[i]/rr[i])-1)) * taper2(rr[i],Rin,Rout) * m.pow((rr[i]/Rin),Rho_Power)*step*stepz * 2. * np.pi * rr[i]
        sigma[i] += m.exp(1./Aspect0/Aspect0 * m.pow((rr[i]/Rin),(-Temp_Power-1.))*(1.0/m.sqrt(1+zz[j]*zz[j]/rr[i]/rr[i])-1)) * taper2(rr[i],Rin,Rout) * m.pow((rr[i]/Rin),Rho_Power)*stepz 
...
#    vphi[i] = m.sqrt(vphi[i])
 ...
 #   if (rr[i] < Rout*0.99) and (rr[i]>Rin*1.01) : #at outer edge /rho cause sigularity
    vphi1[i]= vphi[i] + (1.0 + beta) / beta * C_s * C_s * m.pow((rr[i]/Rin),Temp_Power) * \
                  dr_taper2(rr[i],Rin,Rout)/taper2(rr[i],Rin,Rout)*rr[i]
...
p=pynbody.analysis.profile.Profile(s1.g, nbins=500, ndims=3,max=5)
plt.figure()
