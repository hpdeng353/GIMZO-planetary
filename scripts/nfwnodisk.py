...
#    mu=1.0
    ...
    print np.average(q[:,0]),np.average(q[:,1]),np.average(q[:,2]), 'xyz0'
    ...
    vmag=vvir*np.sqrt(fcx/xfc)

    phi1 = 2 * np.pi * np.random.uniform(0,1,Ngas)
    theta1= np.arccos(2*np.random.uniform(0,1,Ngas)-1)
