...
def randparticle(numpart,rho_power,temp_power,rin,rout,h0,x0,sharp):   #beta goes to infinite reduce to zero BF +
    ...
    normal = 0
    normal1 = 0
    Hmax = h0 * m.pow((rout/rin),(temp_power+3.0)/2.0) * 6
    aspect0=h0/rin
    stepz = 1.0/300*Hmax
...
#    soft =0.02
    ...
    X0=6.*Rin
#Sharp=0.05*X0
    in_over_out = 0.1   #rho_in / rho_out
    ...
    for i in range(0,len(Radius)):
        ...
        if Vt[i]<0:
            ...
            Vt[i]=m.sqrt(Vt[i])
        if (bndparticle == 1) and (Radius[i] < X0):
            id_g[i] = 0
#            Vr= -1.5 * alpha * C_s * C_s / Vt * m.pow((Radius[i]/Rin),Temp_Power)
    ...
    for i in range(0,len(Radius)): # bf in gauss
         ...
         Bt[i]= m.sqrt(8.0 * np.pi *34.55277 /beta *0.9 *Bt[i])*C_s * m.pow(Radius[i]/Rin,Temp_Power/2.0)
