...
zprofile= np.zeros(500)
rr = np.arange(Rin,Rout,step)
zstep= Zmax/500.
zcor  = np.arange(0,Zmax,zstep)
...
rmiddle= (Rmin+Rmax)/2.
aspect=Aspect0*m.pow((rmiddle/Rin),(Temp_Power+1)/2.)
for i in range(0,len(zcor)):
    zprofile[i] = m.exp(1./aspect/aspect*(1./m.sqrt(1.+zcor[i]*zcor[i]/rmiddle/rmiddle)-1))
...
zprofile= zprofile * rho0*taper1(rmiddle,X0,Sharp)*m.pow(rmiddle/Rin,Rho_Power) * 2


vphi = np.zeros(1000)
vphi1 = np.zeros(1000)
internalu = np.zeros(1000)
...
for i in range(0,1000): #rhoprofile is suface density testsigma is surfdensity times geometric factor
    ...
    internalu[i] = C_s * C_s * m.pow((rr[i]/Rin),Temp_Power) * (1.0/beta + 1.0/(gamma -1))
plt.figure()
plt.plot(rr,vphi)
plt.plot(rr,vphi1,'r')
#/zbox/data/hpdeng/gizmout/ngrvmhd1/snapshot_048.hdf5")
s1=pynbody.load("/zbox/data/hpdeng/gizmout/snapshot_000.hdf5")#init/50m20-2.hdf5")#
p=pynbody.analysis.profile.Profile(s1.g, nbins=500, ndims=3,max=10)
pv=pynbody.analysis.profile.VerticalProfile(s1.g,rmin=Rmin,rmax=Rmax,zmax=Zmax,ndims=3)
plt.figure()
...
plt.figure()
plt.plot(pv["rbins"], pv["density"],label='snapshot')
plt.plot(zcor,zprofile,'-r',label='analytical')
plt.title("vertical rho")
plt.legend()


plt.figure()
plt.plot(p["rbins"], p["u"],label='snapshot')
plt.plot(rr,internalu,'-r',label='analytical')
plt.title("u")
plt.legend()
