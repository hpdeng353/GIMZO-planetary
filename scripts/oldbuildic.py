...
def randparticle(numpart,rho_power,temp_power,rin,rout,h0,x0,sharp):   #beta goes to infinite reduce to zero BF +
    import random
    ...
    normal = 0
    normal1 = 0
    Hmax = h0 * m.pow((rout/rin),(temp_power+3.0)/2.0) * 3
    aspect0=h0/rin
    stepz = 1.0/300*Hmax
    zz = np.arange(-Hmax,Hmax,stepz)
    for i in range(0,10000):
        for j in range(0,600):
            normal1 += m.exp(1./aspect0/aspect0 * m.pow((rr[i]/rin),(-temp_power-1.))*(1.0/m.sqrt(1+zz[j]*zz[j]/rr[i]/rr[i])-1)) * taper1(rr[i],x0,sharp) * m.pow((rr[i]/rin),rho_power)*step*stepz * 2. * np.pi * rr[i]

    for i in range(0,10000):
        sigma[i] = taper1(rr[i],x0,sharp) * m.pow((rr[i]/rin),rho_power) * 2.0 *np.pi * rr[i] * m.pow((rr[i]/rin),(temp_power+3.0)/2.0) * h0 * m.sqrt(2.0 * np.pi) #note the final term corespond to the geometry factor 2*pi*r*rho
        rho[i] = taper1(rr[i],x0,sharp) * m.pow((rr[i]/rin),rho_power) * rr[i]
        normal += step * sigma[i]
    for i in range(0,len(rho)):
        if rho[i] > maxrho:
            maxrho = rho[i]
#    maxrho = maxrho / normal1
    while count < numpart :
...
#        rho_x1_y1 = (m.sqrt(rout/x1) + m.sqrt(rin/x1) - m.sqrt(rout * rin / x1 / x1) -1) * x1* m.pow((x1/rin),rho_power) * m.exp(-y1*y1/2./H1/H1)
        rho_x1_y1 = taper1(x1,x0,sharp) * x1* m.pow((x1/rin),rho_power)*m.exp(1./aspect0/aspect0 * m.pow((x1/rin),(-temp_power-1.))*(1.0/m.sqrt(1+y1*y1/x1/x1)-1))
        if rho_x1_y1 > z1 :
            radius.append(x1)
            z.append(y1)
            count += 1
...
#Sharp=0.05*X0
    ...
    Radius,Theta,Z, Normal, Normal1 = randparticle(Ngas,Rho_Power,Temp_Power,Rin,Rout,H0,X0,Sharp)
    ...
    rho0 = diskmass / Normal1
    print 'rho0=',rho0,'thindisk_normal=',Normal,'exact_normal=',Normal1
    ...
    for i in range(0,len(Radius)):
        Vt[i] = V_k * V_k * Rin / Radius[i] + 2.0 / beta * C_s * C_s * m.pow((Radius[i]/Rin),Temp_Power) + \
                ...
                + V_k * V_k /(Radius[i]/Rin) * (-Radius[i]/m.sqrt(Radius[i]*Radius[i]+Z[i]*Z[i])+1.0)*Temp_Power
        Vt[i] = Vt[i] + (1.0 + beta) / beta * C_s * C_s * m.pow((Radius[i]/Rin),Temp_Power) * \
                dr_taper1(Radius[i],X0,Sharp)/taper1(Radius[i],X0,Sharp)*Radius[i]
    ...
    for i in range(0,len(Radius)): # bf in gauss
         Bt[i]= rho0 * m.exp(1./Aspect0/Aspect0 * m.pow((Radius[i]/Rin),(-Temp_Power-1.))*(1.0/m.sqrt(1+Z[i]*Z[i]/Radius[i]/Radius[i])-1)) * taper1(Radius[i],X0,Sharp)*m.pow((Radius[i]/Rin),Rho_Power)
