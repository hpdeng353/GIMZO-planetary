...
def randparticle(numpart,rin,rout,rc):
    import random
    step = 1.0/10000 * (rout - rin)

    rr = np.arange(rin,rout,step)+step
    rho=rr*rr*rc*rc/(rr*rr+rc*rc)
    ...
    while count < numpart :
        ...
        rho_x1 = x1*x1*rc*rc/(x1*x1+rc*rc)
        if rho_x1 > z1 :
            radius.append(x1)
            count += 1
            if (count % 10000 ==0):
    ...
    theta= np.arccos(2*np.random.uniform(0,1,numpart)-1)        
...
def makedisk():
    ...
    rho=0.73e5 #M pc^-3
    Rc=0.52  
    vinf2=4*np.pi*rho*Rc*Rc
    Rout=12.
    vout2=vinf2*(1-Rc/Rout*np.arctan(Rout/Rc))
    StarMass = 4.2e4     #in 1e4 solar mass
    HaloMass = Rout*vout2
    massr=100.
    Ngas =int(Nstar*massr*HaloMass/StarMass)
    print 'h/d=',HaloMass/StarMass,'m ratio=', massr, 'star mass=', StarMass/Nstar,'vinf=', np.sqrt(vinf2),'ngas=', Ngas
    Rs1=2.65
    Rin=0.
    Rin1=0.
    ...
    Radius,Theta,Phi=randstar(Nstar,Rin1,Rout,Rs1)
    ...
    q=np.zeros((Nstar,3))
    vmag=np.sqrt(vinf2*(1.-Rc/Radius*np.arctan(Radius/Rc)))
    sigma=StarMass/2./np.pi/Rc/Rc                        #freeman 1970
    ...
    Radius,Theta,Phi=randparticle(Ngas,Rin,Rout,Rc)
    ...
    temp=StarMass/Nstar/(HaloMass/Ngas)
    print 'mass ratio', temp
