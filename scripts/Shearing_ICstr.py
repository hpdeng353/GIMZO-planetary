...
def makeIC_box():
    ...
    fname='box3d.hdf5'; # output filename 
    ...
    xv_g = np.random.uniform(0,Lx,Ngas)
    yv_g = np.random.uniform(0,Ly,Ngas)
    zv_g = np.random.normal(0.,1.0,Ngas)
#    print  min(zv_g),max(zv_g)
#    for i in range(0,len(zv_g)):
 #       if (abs(zv_g[i])>4):
  #          zv_g[i] *= 0.8
...
#    uv_g=P_desired/((gamma_eos-1.)*rho_desired*np.exp(-zv_g*zv_g/2.)) + 0.*xv_g
    uv_g = 1./(gamma_eos-1.)/gamma_eos+0.*xv_g
   # set the gas IDs: here a simple integer list
    id_g=np.arange(1,Ngas+1)
    zv_g += Lz/2.

#    B0=1#np.sqrt(4*np.pi)*VaKz/Kz
    B0=np.sqrt(8.*np.pi/100)*Cs
    beta=B0*B0/8./np.pi
    beta=1./beta
...
#    print "beta=",beta,"Qz=",Qz,"v0=",v0
    ...
    #by_g=B0+0*xv_g
    bz_g=B0*np.sin(xv_g/Lx*2*np.pi)
...
#    by_g += ample1*np.random.uniform(-1.,1.,Ngas)*B0
 #   vx_g += ample1*np.random.uniform(-1.,1.,Ngas)*Cs
  #  vy_g += ample1*np.random.uniform(-1.,1.,Ngas)*Cs
